 ...
 * @author p_cor
 */
public class MenuComboBoxPanel  extends MyPanel{
    ...
    public MenuComboBoxPanel(){
        super(new FlowLayout(FlowLayout.LEFT));
        
        list = new ArrayList<>();
        combobox = new JComboBox();
        
        label = new MyLabel(GlobalFields.PROPERTIES.getProperty("LABEL_MENU"), 150);
    ...
    public final void load(){
        SwingWorker<Boolean, String> refreshworker = new SwingWorker<Boolean, String>() {
            ...
            protected Boolean doInBackground(){
                ...
                response = RestClient.get("menus?status=true");
    ...
    public Menu getComboBoxValue(){
        Menu menu = (Menu)combobox.getSelectedItem();
        return menu;
    }
    
    public void setComboBoxValue(Menu menu){
        ComboBoxModel model = combobox.getModel();
        
        int size = model.getSize();
        
        for(int i = 0; i < size; i++){
            Menu temp = (Menu)model.getElementAt(i);
            if(menu.getId() == temp.getId()){
                model.setSelectedItem(menu);
            }
        }
    }
