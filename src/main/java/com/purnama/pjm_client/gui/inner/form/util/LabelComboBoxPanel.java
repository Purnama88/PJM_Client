...
public class LabelComboBoxPanel extends JPanel{
    ...
    public LabelComboBoxPanel(){
        super(new FlowLayout(FlowLayout.LEFT));
        
        list = new ArrayList<>();
        combobox = new JComboBox();
        
        label = new MyLabel(GlobalFields.PROPERTIES.getProperty("LABEL_LABEL"), 150);
    ...
    public final void load(){
        SwingWorker<Boolean, String> refreshworker = new SwingWorker<Boolean, String>() {
            ...
            protected Boolean doInBackground(){
                ...
                response = RestClient.get("labels?status=true");
    ...
    public Label getComboBoxValue(){
        return (Label)combobox.getSelectedItem();
    }
    
    public void setComboBoxValue(Label label){
        ComboBoxModel model = combobox.getModel();
        
        int size = model.getSize();
        
        for(int i = 0; i < size; i++){
            Label temp = (Label)model.getElementAt(i);
            if(label.getId() == temp.getId()){
